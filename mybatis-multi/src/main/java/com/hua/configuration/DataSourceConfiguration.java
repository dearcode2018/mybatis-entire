...
public class DataSourceConfiguration {
	...
	/* 事务默认超时时间，单位: 秒 */
	@Value("${datasource.default.tx.timeoutSecond:10}")
	private int defaultTxTimeoutSecond;
	
	/* Statement默认超时时间，单位: 秒 */
	@Value("${datasource.default.statement.timeoutSecond:5}")
    ...
    public DynamicDataSource dynamicDataSource() {
    	...
    	final DataSourceProperties defaultDSProperties = dynamicDataSourceProperties.getDatasource().get(DataSourceNames.DB1);
    	// 默认数据源
    	dataSource.setDefaultTargetDataSource(buildDataSource(defaultDSProperties));
    	// 多个数据源
    ...
    private Map<Object, Object> getDynamicDataSource() {
    	final Map<String, DataSourceProperties> dsProperties = dynamicDataSourceProperties.getDatasource();
    	final Map<Object, Object> targetDataSources = new HashMap<>(dsProperties.size());
    	dsProperties.forEach((k, v) -> targetDataSources.put(k, buildDataSource(v)));
	 ...
	 * @author qianye.zheng
	 */
	 private DruidDataSource buildDataSource(final DataSourceProperties properties) {
